...
function updateLoader(currentValue) {
    if (currentValue < 100) {
        document.querySelector('.title').textContent = currentValue;
        setTimeout(() => updateLoader(currentValue + 1), 20);
    } else {
        const loader = document.querySelector('.title');
        loader.textContent = `${currentValue} lvl`;
        loader.classList.add('lvl-up');
    }
}
updateLoader(0);